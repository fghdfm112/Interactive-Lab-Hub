#!/usr/bin/env python3
# listen.py - Listen to microphone and return recognized speech
...
try:
    ...
    with sd.RawInputStream(samplerate=args.samplerate,
        ...
        print("Listening… speak now (Ctrl+C to quit)", file=sys.stderr)
        print("#" * 60, file=sys.stderr)
        ...
        while True:
            ...
            if rec.AcceptWaveform(data):
                ...
                # Print what was heard to stderr for debugging
                print(f"[heard] {text}", file=sys.stderr)

                # Print the recognized text to stdout and exit
                print(text)
                sys.stdout.flush()
                sys.exit(0)

            else:
                # Check partial results for faster reaction
                ...
                if partial and partial != last_text:
                    ...
                    # Same text detected, increment silence counter
                    silence_count += 1
                    # After detecting the same text for ~2 seconds (about 25 blocks), accept it
                    if silence_count > 25 and partial:
                        print(f"[timeout - accepting] {partial}", file=sys.stderr)
                        print(partial)
                        sys.stdout.flush()
                        sys.exit(0)
